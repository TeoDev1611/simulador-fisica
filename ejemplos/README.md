...
Esta carpeta contiene 16 archivos JSON de escenas listas para importar en el simulador. Cada uno demuestra el uso del código y sirve para poner a prueba las distintas funcionalidades del motor físico. Puedes cargarlos directamente en el simulador usando el botón de **📂 Importar**.

## 🟢 Nivel Básico (Ideal para empezar)
- **`2_collarin_resorte.json`**: Un anillo ensartado en un riel conectado a un resorte que lo empuja hacia un lado.
- **`3_rampa_y_obstaculo.json`**: Una caja deslizándose por una rampa lista para chocar contra un obstáculo.
...
- **`10_coche_amortiguado.json`**: Un carro simplificado construido con ruedas unidas por resortes a un chasis, amortiguando la caída.
- **`12_catapulta_balancin.json`**: Un balancín físico con un contrapeso diseñado para lanzar un proyectil esférico.

## 🎓 Nivel Pruebas y Exámenes (ESPE / Dinámica de Partículas)
- **`13_puente_colgante.json`**: Estructura tipo puente colgante hecha de múltiples segmentos unidos por juntas para estudiar tensión distribuida.
