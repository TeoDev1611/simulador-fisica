...
export function usePlanckWorld(gravityMagnitude = DEFAULT_GRAVITY) {
  ...
  function startMouseDrag(id, x, y, isPaused = false) {
  ...
  function updateMouseDrag(x, y, isPaused = false) {
    if (isPaused && m_draggedBody) {
      m_draggedBody.setPosition(Vec2(x, y))
      m_draggedBody.setLinearVelocity(Vec2(0, 0))
      m_draggedBody.setAngularVelocity(0)
      syncTransforms()
    } else if (m_mouseJoint) {
      m_mouseJoint.setTarget(Vec2(x, y))
    }
  }
  ...
  function applyImpulse(id, magnitude, angleDeg) {
    ...
    const rad = (angleDeg * Math.PI) / 180
    entry.body.setAwake(true)
    entry.body.applyLinearImpulse(
      Vec2(magnitude * Math.cos(rad), magnitude * Math.sin(rad)),
      entry.body.getWorldCenter(),
      true
    )
  }
  ...
  function step(dt = 1 / 60) {
    try {
      ...
      const toRemove = []
      for (const entry of bodies) {
        if (entry.kind === 'box' && entry.position.y < -25) {
